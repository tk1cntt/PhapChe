...
Provide admin operational visibility for the legal workflow: dashboard counts, request filters, workload views, SLA timestamp/aging tracking, and per-request audit timeline. Phase 7 does not add advanced analytics, SLA escalation automation, billing analytics, or customer-facing reporting.
...
- `.planning/PROJECT.md` — Project constraints: legal accuracy, security, workflow integrity, traceability, and MVP scope.
...
- `.planning/phases/01-foundation/01-CONTEXT.md` — Server-side RBAC, append-only audit, backend-owned workflow transitions.
...
- `src/lib/security/rbac.ts` — Request/workspace access rules.
...
- `getAllowedTransitions` / workflow transition model from `src/lib/workflow/request-workflow.ts`: use status lifecycle as source for aging/status labels, not frontend-only logic.
...
- Prisma is accessed via `src/lib/prisma.ts` and models in `prisma/schema.prisma`.
- RBAC is server-side via `src/lib/security/rbac.ts`; UI navigation cannot be security boundary.
